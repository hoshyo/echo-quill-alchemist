...
- 目标 skill 落盘路径：<本 skill 同级目录>/<novel-slug>-quill/
...
> 本 skill 同级目录 = 本 SKILL.md 所在目录的父目录（即全局 skills 安装根，如类 Unix 上常见的 `~/.agents/skills/`、Windows 上常见的 `<用户主目录>/.agents/skills/`）。主 agent 在前置环境校验时通过自身 SKILL.md 的绝对路径反推出该根目录，全程不要把任何具体的用户名 / 操作系统路径写死进任何文件。
```
...
确认"本 skill 同级目录"（由本 SKILL.md 所在目录的父目录推得）存在且可写。`<novel-slug>-quill/` 子目录若**已存在**：停下询问用户"目标路径已有同名 skill，三选一：(A) 覆盖（旧 skill 备份到 `<...>-quill.bak-<timestamp>/`） (B) 改用新名 (C) 取消训练"。**不得**默默覆盖。
...
- 评分细则：<本 skill 安装目录>/references/scoring-rubric.md（必读）
